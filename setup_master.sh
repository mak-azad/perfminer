...
echo "Updating system again and installing Python 3.10..."
sudo apt-get update && sudo apt-get install -y python3.10

# Display the installed Python version
echo "Python version installed:"
python3.10 --version
# Install Python 3.10 distutils, required for pip installation
echo "Installing python3.10-distutils..."
sudo apt-get install -y python3.10-distutils

# Download get-pip.py script and install pip for Python 3.10
echo "Downloading get-pip.py and installing pip for Python 3.10..."
...
# Using parallel-ssh to update and install Python 3.10 on multiple hosts
# echo "Updating and installing Python 3.10 on all hosts listed in sshhosts..."
# parallel-ssh -i -h sshhosts 'sudo apt-get update && sudo apt-get install -y software-properties-common && sudo add-apt-repository -y ppa:deadsnakes/ppa && sudo apt-get update && sudo apt-get install -y python3.10'
